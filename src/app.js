...
function renderTopNav() {
  ...
  return `
    <header class="top-nav">
      <button class="brand-mark" data-route="/dashboard" type="button" aria-label="${labels.homeAria}">
        <span>Q</span>
...
function renderBottomNav() {
  const labels = t();
  return `
    <nav class="bottom-nav" aria-label="${labels.mobileNavigation}">
      ${navButton("/dashboard", "home", labels.home)}
      ${navButton("/lessons", "list", labels.lessons)}
      ${navButton("/progress", "chart", labels.progress)}
      ${navButton("/profile", "user", labels.profile)}
...
function renderLanguageSwitcher(compact = false) {
  return `
    <div class="lang-switch ${compact ? "compact" : ""}" aria-label="Language switcher">
      <button class="${state.language === "kk" ? "active" : ""}" data-lang="kk" type="button">Қазақша</button>
      <button class="${state.language === "ru" ? "active" : ""}" data-lang="ru" type="button">Русский</button>
    </div>
  `;
...
function renderTopNav() {
  ...
  return `
    <header class="top-nav">
      <button class="brand-mark" data-route="/dashboard" type="button" aria-label="Qadam home">
        <span>Q</span>
        <strong>${labels.brand}</strong>
      </button>
      <nav aria-label="Primary">
        ${navButton("/dashboard", "⌂", labels.home)}
        ${navButton("/lessons", "▤", labels.lessons)}
        ${navButton("/progress", "▥", labels.progress)}
        ${navButton("/profile", "◉", labels.profile)}
      </nav>
      ${renderLanguageSwitcher(true)}
    </header>
  `;
...
function navButton(path, icon, label) {
  const active = normalizePath(location.pathname) === path;
  return `
    <button class="nav-item ${active ? "active" : ""}" data-route="${path}" type="button">
...
function renderLanding() {
  ...
  return pageShell(
    `
      ...
      <section class="band benefits-band" aria-label="Benefits">
        <div class="content-grid three">
          ${labels.benefits
            .map(
              ([title, text]) => `
